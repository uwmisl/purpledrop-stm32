...
struct ElectrodeEnableMsg {
    ...
    ElectrodeEnableMsg() : groupID(0), setting(0), values{0} {}

    ElectrodeEnableMsg(uint8_t *buf, uint32_t length) : ElectrodeEnableMsg() {
        fill(buf, length);
    }
...
struct SetPwmMsg {
    ...
    SetPwmMsg() : channel(0), duty_cycle(0) {}
...
struct CalibrateCommandMsg {
    ...
    CalibrateCommandMsg() : command(0) {}

    CalibrateCommandMsg(uint8_t *buf, uint32_t length) : CalibrateCommandMsg() {
        fill(buf, length);
    }
...
struct GpioControlMsg {
    ...
    GpioControlMsg() : pin(0), flags(0) {}

    GpioControlMsg(uint8_t *buf, uint32_t length) : pin(0), flags(0) {
        fill(buf, length);
    }
...
struct FeedbackCommandMsg {
    ...
    FeedbackCommandMsg() : 
        target(0.0),
        mode(0),
        measureGroupsPMask(0),
        measureGroupsNMask(0),
        baseline(0) 
        {}
    
    FeedbackCommandMsg(uint8_t *buf, uint32_t length) : FeedbackCommandMsg() {
        fill(buf, length);
    }
