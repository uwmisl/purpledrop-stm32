...
template<
    uint32_t Sector0Num, uint32_t Sector0Addr, uint32_t Sector0Size,
    uint32_t Sector1Num, uint32_t Sector1Addr, uint32_t Sector1Size>
struct SafeFlashPersist {
    ...
    bool read(uint8_t **buf, uint32_t *length) {
        if(validate_page(0)) {
            *buf = (uint8_t*)Sector0Addr + 8;
            *length = *((uint32_t *)Sector0Addr + 1) - OVERHEAD_SIZE;
            return true;
        } else if(validate_page(1)) {
            *buf = (uint8_t*)Sector1Addr + 8;
            *length = *((uint32_t *)Sector1Addr + 1) - OVERHEAD_SIZE;
            return true;
        }
        return false;
    }
    ...
    bool write(uint8_t *buf, uint32_t length) {
        ...
        uint32_t crc;
        if(validate_page(0)) {
            dst_addr = (uint8_t*)Sector1Addr;
            dst_sector = Sector1Num;
            dst_size = Sector1Size;
            page_to_invalidate = 0;
        } else {
            dst_addr = (uint8_t*)Sector0Addr;
            dst_sector = Sector0Num;
            dst_size = Sector0Size;
    ...
    void invalidate_page(uint8_t page) {
        ...
        if(page == 0) {
            p = (uint8_t*)Sector0Addr;
        } else if(page == 1) {
            p = (uint8_t*)Sector1Addr;
        } else {
            return;
        }
        // Write the header byte to all ones to invalidate
        flash::write(p, (uint8_t*) &write_value, sizeof(write_value));
    }
    ...
    bool validate_page(uint8_t page) {
        ...
        if(page == 0) {
            p = (uint8_t *)Sector0Addr;
            sector_size = Sector0Size;
        } else if(page == 1) {
            p = (uint8_t *)Sector1Addr;
            sector_size = Sector1Size;
        } else {
            return false;
        }
